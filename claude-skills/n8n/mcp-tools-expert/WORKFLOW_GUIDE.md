...
- Use `chainLlm` for simple AI tasks — use `openAi` node (resource: text) instead
- Confuse `form` (multi-page continuation) with `formTrigger` (the actual trigger)
- Claim workflows "work" based on validation alone — ALWAYS verify actual execution
- Skip checking execution count after n8n_test_workflow calls
